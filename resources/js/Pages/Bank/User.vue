...
// ✅ Debounced search
const searchBankUsers = debounce(() => {
  router.get(route('bank.user'), { search: search.value }, {
    preserveState: true,
    preserveScroll: true,
    replace: true
  })
          ...
          <div class="p-6 bg-white dark:bg-gray-800">
            ...
            <div class="flex flex-wrap items-center justify-between gap-2 mb-6">
              ...
              <input
                ...
                class="px-3 py-2 text-sm border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
              />
            </div>
              ...
              <table class="w-full text-sm text-left text-gray-600 rtl:text-right dark:text-gray-300">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td>{{ item.user?.name_th || '-' }} {{ item.user?.surname_th || '-' }}</td>
                    <td class="px-4 py-2">{{ item.user?.email }}</td>
                    <td class="px-4 py-2">
                      <Link
                        :href="route('bank.account', item.student_id)"
