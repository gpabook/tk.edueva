<template>
    <div class="max-w-xl p-4 mx-auto">
      <h1 class="mb-4 text-2xl font-semibold">Edit Class Level</h1>
  ...
  const { props } = usePage()
  const { classLevel } = props.value

  const form = useForm({
    code: classLevel.code,
    name_en: classLevel.name_en,
    name_th: classLevel.name_th,
  })

  function submit() {
    form.put(route('class-levels.update', classLevel.id))
  }
  </script>
